...
import { useDoctors } from "repositories/review-repository";
...
function Doctors({ t }) {
  ...
  const { isLoading, data, error } = useDoctors(
    pageNumber,
    pageSize,
    searchTerm === null ? null : decodeURIComponent(searchTerm)
  );
      ...
      formatter: (cellContent, row) => {
        if (!isLoading)
          return (
            <Link
              to={{
                pathname: constants.routes.superAdmin.doctorReviews.replace(
                  ":doctorId",
                  encodeId(row.id)
                ),
  ...
  return (
    <>
      ...
      {!isLoading && totalItems === 0 && (
        <Empty Message={t("superAdmin.noDoctorFound")} />
      )}
    </>
  );
}
