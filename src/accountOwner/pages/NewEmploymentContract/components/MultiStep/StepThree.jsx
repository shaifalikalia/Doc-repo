...
function StepThree({ contractData, contractFormDisplayData, setFormDetails, handleSwitch, t }) {
  ...
  if (contractData) {
    contractFormDisplayData.forEach(v => {
      if ((v.stepNumber == 1 || v.stepNumber == 2) && v.isMainSection) {
        countFields++;
      }
    });
  }
    ...
    <div className={styles['step-form-wrapper']}>
      ...
      }
      {
        contractFormDisplayData.map((data, key) => (
          data.isHtmlContent && data.stepNumber == 3 && (data.inputType == null || data.inputType == 4) && (<Card
            radius='10px'
            marginBottom='10px'
            padding="30px"
